...
* [`playground-web/`](./playground-web/) — Web playground with piano roll, examples, and decompile. Run `bun run build:web` from `compiler-js/` to copy the library, then open or serve `playground-web/`.
* [`tampermonkey/`](./tampermonkey/) — **Gemini LMP to MIDI** userscript. Install in Tampermonkey to add a “Download MIDI” flow on Gemini when you paste or generate LMP. [Install raw script](https://github.com/zunaidFarouque/LMP-Specification/raw/main/tampermonkey/gemini-lmp-download.user.js).
